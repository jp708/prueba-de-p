...
function refreshAudienceInterface() {
    const businessSections = document.querySelectorAll(".business-only");
    const professionalSections = document.querySelectorAll(".professional-only");

    businessSections.forEach(section => {
        const visible = currentAudience === "business";
        section.setAttribute("aria-hidden", visible ? "false" : "true");
    });

    professionalSections.forEach(section => {
        const visible = currentAudience === "professional";
        section.setAttribute("aria-hidden", visible ? "false" : "true");
    });
}
...
function setAudience(audience, persist = true) {
    ...
    showHeroSlide(audience === "professional" ? 1 : 0);
    startHeroAutoplay();
}
...
function requestAudienceChange(audience, onDone) {
    ...
    window.setTimeout(() => {
        if (audienceTransition) audienceTransition.classList.remove("is-active");
        body.classList.remove("audience-changing");
        audienceTransitionRunning = false;
